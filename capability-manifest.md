...
| C059 | unalz (`unalz.exe`) — ALZip `.alz` archives | behavior | code | none found (directory check only) | DONE | | PR [#233](https://github.com/baileyrd/rusty_extract/pull/233), tests `detection::alz_probe::tests`, `extract::alz::tests::bundled_ini_produces_source_matching_command_line` |
...
| C067 | cicdec (`cicdec.exe`) — Clickteam Install Creator installers | behavior | code | none found (directory check only) | DONE | | PR [#208](https://github.com/baileyrd/rusty_extract/pull/208), test `extract::cic::tests::matches_source_invocation` |
| C068 | GARbro (`GARbro.Console.exe`) — 500+ visual-novel/game-engine archive formats | behavior | code | none found (directory check only) | DONE | | PR [#205](https://github.com/baileyrd/rusty_extract/pull/205), test `extract::garbro::tests::matches_source_invocation` |
...
| C070 | xor (`xor.exe`) — byte-XOR decode of Ghost Installer's overlay-extracted CAB blob | behavior | code | none found (directory check only) | DONE | | PR [#207](https://github.com/baileyrd/rusty_extract/pull/207), test `extract::xor::tests::matches_source_invocation` |
...
| C072 | FSB extractor (`fsbext.exe`) — FMOD Sample Bank (`.fsb`) | behavior | code | none found (directory check only) | DONE | | PR [#210](https://github.com/baileyrd/rusty_extract/pull/210), test `extract::fsb::tests::matches_source_invocation` |
...
| C078 | unisz (`unisz.exe`) — ISZ compressed ISO (stage 1 of disk-image conversion) | behavior | code | none found (directory check only) | DONE | | PR [#209](https://github.com/baileyrd/rusty_extract/pull/209), test `extract::isz::tests::matches_source_invocation` |
...
| C102 | uif2iso (`uif2iso.exe`) — MagicISO `.uif` images (stage-1 conversion to ISO) | behavior | code | none found (directory check only) | DONE | | PR [#211](https://github.com/baileyrd/rusty_extract/pull/211), test `extract::uif::tests::matches_source_invocation` |
| C103 | umodel (`umodel.exe`/`unreal.exe`) — Unreal Engine packages | behavior | code | none found (directory check only) | DONE | | PR [#214](https://github.com/baileyrd/rusty_extract/pull/214), test `extract::unreal::tests::matches_source_invocation` |
...
| C107 | dark / WiX Toolset (`dark.exe`) — WiX MSI-based installers | behavior | code | none found (directory check only) | DONE | | PR [#213](https://github.com/baileyrd/rusty_extract/pull/213), test `extract::wix::tests::matches_source_invocation` |
| C108 | WolfDec (`WolfDec.exe`) — Wolf RPG Editor game archives | behavior | code | none found (directory check only) | DONE | | PR [#215](https://github.com/baileyrd/rusty_extract/pull/215), test `extract::wolf::tests::matches_source_invocation` |
| C109 | Info-ZIP UnZip (`unzip.exe`) — generic ZIP fallback when 7-Zip fails | behavior | code | none found (directory check only) | DONE | | PR [#218](https://github.com/baileyrd/rusty_extract/pull/218), test `extract::unzip::tests::matches_source_invocation` |
| C110 | unzoo (`unzoo.exe`) — `.zoo` Zoo archives | behavior | code | none found (directory check only) | DONE | | PR [#217](https://github.com/baileyrd/rusty_extract/pull/217), test `extract::zoo::tests::matches_source_invocation` |
| C111 | zpaq (`zpaq.exe`) — `.zpaq` ZPAQ archives | behavior | code | none found (directory check only) | DONE | | PR [#216](https://github.com/baileyrd/rusty_extract/pull/216), test `extract::zpaq::tests::matches_source_invocation` |
| C112 | upx (`upx.exe`) — UPX-packed executables (unpack) | behavior | code | none found (directory check only) | DONE | | PR [#219](https://github.com/baileyrd/rusty_extract/pull/219), test `extract::upx::tests::matches_source_invocation` |
...
| C146 | DAA→ISO conversion has no pre-existing-output-file check. **Known quirk, verify still present**: matches the documented "converting to iso fails when iso already exists" bug. | behavior | code | none found (directory check only) | DONE | | PR [#225](https://github.com/baileyrd/rusty_extract/pull/225), test `extract::daa::tests::matches_source_invocation` |
