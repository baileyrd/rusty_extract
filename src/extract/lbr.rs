//! lbrate (`lbrate.exe`) — `.lbr`/`.lzr`/`.lqr` LBR archives, dispatched
...
/// The bundled `def/lbr.ini` plugin definition, verbatim.
pub const BUNDLED_INI: &str = include_str!("../../def/lbr.ini");
...
mod tests {
    ...
    /// Parity test for capability C126: parsing the bundled `def/lbr.ini`
    ...
    fn bundled_ini_produces_source_matching_command_line() {
        ...
        let config = PluginConfig::parse(&ini, "lbr");

        assert_eq!(config.executable, "lbrate.exe");
        ...
        assert_eq!(command_line, r#"lbrate.exe "C:\downloads\archive.lbr""#);
    }
}
