//! unar / TheUnarchiver (`unar.exe`) — StuffIt (`.sit`/`.sitx`) archives,
...
/// The bundled `def/sit.ini` plugin definition, verbatim.
pub const BUNDLED_INI: &str = include_str!("../../def/sit.ini");
...
mod tests {
    ...
    /// Parity test for capability C134: parsing the bundled `def/sit.ini`
    ...
    fn bundled_ini_produces_source_matching_command_line() {
        ...
        let config = PluginConfig::parse(&ini, "sit");

        assert_eq!(config.executable, "unar.exe");
        ...
        assert_eq!(
            command_line,
            r#"unar.exe -o "C:\downloads\archive_unpacked" "C:\downloads\archive.sit""#
        );
    }
}
